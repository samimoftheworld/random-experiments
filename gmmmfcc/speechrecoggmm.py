...
le=length/1000+1 #take lower limit of the length/1000 
seg=1 #declare segment
for i in range(0,le):
	if(i==le):
		le1=(i+1)*1000+1 #initial time for the last frame
		seg=sound[le1:length] #the remaining time stored here
		seg.export("/home/samim/chunk%d.wav"%i+1, format="wav") #export the segment
	else:
		le1=i*1000+1 #initial time
		le2=(i+1)*1000 #final time
...
for i in xrange(1,le):
	print 'So the %d file is calssified as'%(i-1),x[i]
